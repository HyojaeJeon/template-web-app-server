...
/**
 * TaxIdInput Component - Tax ID with Country Selector
 * PhoneInput과 동일한 디자인 패턴 적용
 *
 * Features:
 * - 한 줄 레이아웃: 국가 선택 버튼 (왼쪽) + Tax ID 입력 (오른쪽)
 * - Portal 기반 플로팅 드롭다운
 * - Real-time validation with checksum verification
 * - Multilingual error messages
 * - Auto-formatting based on country
 * - Support for Vietnam MST, Korea BRN, US EIN, Japan CN, Singapore UEN
 */
...
import {
  ...
  validateTaxId,
  getTaxIdPlaceholder,
} from '@/shared/utils/taxIdValidation';

// 국가 목록 (Local 우선)
const COUNTRIES = [
  { code: 'VN', name: 'Vietnam', nameKo: 'Local', flag: '🇻🇳', taxIdName: 'MST' },
  { code: 'KR', name: 'South Korea', nameKo: '한국', flag: '🇰🇷', taxIdName: '사업자등록번호' },
  { code: 'US', name: 'United States', nameKo: '미국', flag: '🇺🇸', taxIdName: 'EIN' },
  { code: 'JP', name: 'Japan', nameKo: '일본', flag: '🇯🇵', taxIdName: '法人番号' },
  { code: 'SG', name: 'Singapore', nameKo: '싱가포르', flag: '🇸🇬', taxIdName: 'UEN' },
];
...
const TaxIdInput = ({
  ...
  const filteredCountries = searchTerm
    ? COUNTRIES.filter(country =>
        ...
        country.taxIdName.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : COUNTRIES;
  ...
  const getErrorMessage = (errorKey) => {
    ...
    const fallbackMessages = {
      'taxId.required': '세금 번호를 입력해주세요',
      'taxId.vietnam.invalidLength': 'Local MST는 10자리 또는 13자리여야 합니다',
      'taxId.vietnam.invalidProvinceCode': '유효하지 않은 지역 코드입니다',
      'taxId.vietnam.invalidChecksum': '유효하지 않은 MST입니다 - 세금 번호를 확인해주세요',
      'taxId.vietnam.invalidSubsidiaryCode': '유효하지 않은 자회사 코드입니다 (001-999)',
      'taxId.korea.invalidLength': '사업자등록번호는 10자리여야 합니다',
      'taxId.korea.invalidChecksum': '유효하지 않은 사업자등록번호입니다',
      'taxId.us.invalidLength': '미국 EIN은 9자리여야 합니다',
      'taxId.us.invalidPrefix': '유효하지 않은 EIN 접두사입니다',
      'taxId.japan.invalidLength': '일본 법인번호는 13자리여야 합니다',
      'taxId.japan.invalidChecksum': '유효하지 않은 법인번호입니다',
      'taxId.singapore.invalidFormat': '유효하지 않은 싱가포르 UEN 형식입니다',
    };
  ...
  const validateValue = (val, countryCode = selectedCountry.code) => {
    ...
    const result = validateTaxId(val, countryCode);
  ...
  const handleCountrySelect = (country, e) => {
    ...
    // Tax ID가 있으면 새 국가 코드로 재검증
    if (taxIdValue) {
      validateValue(taxIdValue, country.code);
    }
  };

  // Tax ID 입력 핸들러 - 입력 시 즉시 유효성 검사
  ...
  const handleBlur = () => {
    ...
    if (validateOnBlur && taxIdValue) {
      validateValue(taxIdValue);
    }
  };
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      ...
      const clickedInsidePortal = e.target.closest('[data-taxid-select-portal="true"]');
  ...
  // 국가 정보
  const countryData = TAX_ID_COUNTRIES[selectedCountry.code];
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`} ref={selectRef}>
      ...
      <div className="flex gap-2 items-center">
        ...
        <div className="flex-1">
          <input
            ...
            onBlur={handleBlur}
            placeholder={placeholder || getTaxIdPlaceholder(selectedCountry.code)}
            disabled={disabled}
            required={required}
            className={`
        ...
        >
          ...
          <div className="sticky top-0 p-2 border-b border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 z-10">
            <input
              ...
              placeholder={t('taxId.searchCountry', { defaultValue: '국가 검색...' })}
              value={searchTerm}
          ...
          <div className="py-1">
            {filteredCountries.length === 0 ? (
              <div className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">
                {t('taxId.noCountryFound', { defaultValue: '검색 결과가 없습니다' })}
              </div>
            ) : (
              filteredCountries.map((country, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-gray-600 dark:text-gray-400 text-xs">{country.taxIdName}</span>
                  </button>
                );
              })
            )}
      ...
      {/* 유효성 검사 성공 표시 */}
      {!displayError && isValid && taxIdValue && (
        <p className="text-xs text-emerald-600 dark:text-emerald-400">
          ✓ {t('taxId.valid', { defaultValue: '유효한 세금 번호입니다' })}
        </p>
      )}
      ...
      {!displayError && !isValid && (
        <p id={`${generatedId}-helper`} className="text-xs text-gray-500 dark:text-gray-500">
          {helperText || `${t('taxId.formatHint', { defaultValue: '형식' })}: ${countryData?.format || ''}`}
        </p>
      )}
    </div>
  );
